...
```text
dili1324/weather-agent-07y0bean
```
...
```text
POST https://api.github.com/repos/dili1324/weather-agent-07y0bean/actions/workflows/weather-agent.yml/dispatches
```
...
Create a fine-grained personal access token for the repository `dili1324/weather-agent-07y0bean`.
...
curl -L \
  ...
  https://api.github.com/repos/dili1324/weather-agent-07y0bean/actions/workflows/weather-agent.yml/dispatches \
  -d '{"ref":"main"}'
```
...
```text
URL: https://api.github.com/repos/dili1324/weather-agent-07y0bean/actions/workflows/weather-agent.yml/dispatches
Method: POST
...
- Use a fine-grained PAT scoped only to `dili1324/weather-agent-07y0bean`.
